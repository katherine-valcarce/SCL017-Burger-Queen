  ...
  return (
    <>
      <div className="orderContainer">
        <div>
          {cartItems.length === 0 && (
            <h5 className="textAdd">Agrega productos al resumen del pedido </h5>
          )}
        </div>
          ...
          {cartItems.map((item) => (
            <div key={item.id} className="selectItemOrder">
              ...
              <div className="qtyOrder">
                <button onClick={() => onAdd(item)} className="btnAR add">            
                  +
                </button>
                <div className="textQuantity"> {item.qty} </div>
                <button onClick={() => onRemove(item)} className="btnAR remove">
                  -
                </button>
              </div>
              <div className="quantyPriceItem">${item.price}</div>
              <button
            ...
            <>
              <div className="rowPrice">
                <div className="textBill">                  
                  <b className="textTotalOrder"> Total cuenta </b>
                </div>
                <div className="textBill text-right">
                  <b>$ {itemsPrice} </b>
                </div>
              </div>
            </>
